...
#define DESKTOP_CMD_EXEC_SETTINGS_VERSION 0.6
	...
	if (fileExists) {
		...
		else
		{
			...
			if (error) {
				//no value? assume std
				noInstanceKeysFlag = TRUE;
				self->priv->widthRatio = 1.0;

				g_warning("no widthRatio on key file, assuming 1.0");

				g_error_free (error);
				error = NULL;
			}

			self->priv->heightRatio = g_key_file_get_double (keyFile, self->priv->widgetID, "heightRatio", &error);//AP
			if (error) {//AP
				//no value? assume std//AP
				noInstanceKeysFlag = TRUE;//AP
				self->priv->heightRatio = 1.0;//AP

				g_warning("no heightRatio on key file, assuming 1.0");//AP
				g_error_free (error);//AP
				error = NULL;//AP
			}//AP
...
//  		g_warning("records empty, filling with pre-loaded vals");
		...
		gchar * p_commands[] = {
		"uptime | awk 'NF == 7 {print $2 \" \" $3; exit 0}; NF == 8 {print $2 \" \" $3; exit 0}; NF == 10 {print $3 \" \" $4 \" \" $5; exit 0}; {print $3 \" \" $4 \" \" $5}'",
	...
	g_key_file_set_double (keyFile, self->priv->widgetID, "heightRatio", self->priv->heightRatio);//AP
...
// 	GtkSizeGroup *group = GTK_SIZE_GROUP (gtk_size_group_new (GTK_SIZE_GROUP_HORIZONTAL));
	...
	gtk_label_set_line_wrap (GTK_LABEL (self->priv->cmdResult_lb), TRUE);//AP
	...
	gtk_misc_set_alignment (GTK_MISC (self->priv->cmdTitle_lb), 0, 0);//Changed Alignment to Top-Left //AP
	gtk_misc_set_alignment (GTK_MISC (self->priv->cmdResult_lb), 0, 0);//Changed Alignment to Top-Left //AP
...
// 	g_warning ("popen(%s) status:%p",self->priv->instanceCmd,fp);//AP
...
//	while (fgets (line, sizeof line, fp)) {//Needed to change this "line-based" management for output //AP
	if (l=fread (line, 1, sizeof line, fp)) {//to this "buffer-based" for multiline handling in widget //AP
		line[l-1]='\000';//AP
	 	g_warning ("l=%d line=%s",l,line);//AP
//		result = g_strstrip (line);//AP
//	 	g_warning ("result=%s",result);//AP
//		gtk_label_set_text (GTK_LABEL (self->priv->cmdResult_lb), result);//AP
...
//	hints.max_height = SIZE_HEIGHT_LINE;//AP
	hints.max_height = SIZE_HEIGHT_LINE*10;//AP

	gtk_window_set_geometry_hints (GTK_WINDOW (self), self->priv->homeWidget, &hints, GDK_HINT_MIN_SIZE | GDK_HINT_MAX_SIZE);

	gtk_widget_set_size_request (GTK_WIDGET (self), (int)(SIZE_WIDTH_ALL*0.118), SIZE_HEIGHT_LINE);
...
// 		GtkWidget *content_area = gtk_dialog_get_content_area (GTK_DIALOG (dialog));
		...
		GtkWidget *heightLabel = gtk_label_new ("Height:");//AP
		...
		if (setDialogReturn == GTK_RESPONSE_ACCEPT)
		{
			...
			self->priv->heightRatio = gtk_range_get_value(GTK_RANGE(heightSelect));//AP
